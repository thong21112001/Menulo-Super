...
namespace Menulo.Pages.RestaurantTables
{
    public class CreateModel : PageModel
    {
        ...
        [BindProperty]
        public ResTableRequest.Create Input { get; set; } = new();


        public CreateModel(IUnitOfWork uow, UserManager<ApplicationUser> userManager, IResTablesService service)
        {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var dto = new CreateResTableDto(
                Input.RestaurantId,
                Input.TableQuantity ?? 1,
                Input.Description
            );

            await _svc.CreateAsync(dto);
            TempData.SetSuccess("Tạo mới thành công!");
